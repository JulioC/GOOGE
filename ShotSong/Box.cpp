...
Box::Box(Game* game, const char* filename, const Vector& position, const Vector& size):
Sprite::Sprite(game, filename, position) {
    _boundings[0] = position;
    _boundings[1] = position + size;
}

Box::Box(Game* game, const char* filename, const Vector& position, const Vector& size, const Vector& framesize):
Sprite::Sprite(game, filename, position, framesize) {
    _boundings[0] = position;
    _boundings[1] = position + size;
}
